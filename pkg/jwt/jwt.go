...
type Options struct {
	...
	// Define the token's refresh time, Can't refresh after the refresh time.
...
func (j *jwt) cacheKey(identity interface{}) string {
	return fmt.Sprintf(defaultIdentityKey, j.issuer, String(identity))
}
...
func (j *jwt) saveIdentity(identity, jid interface{}) error {
	...
	return j.adapter.Put(j.getCtx(), j.cacheKey(identity), String(jid), time.Duration(math.Max(float64(j.expiredTime), float64(j.refreshTime))))
}
...
func (j *jwt) removeIdentity(identity interface{}) error {
	...
	return j.adapter.Delete(j.getCtx(), j.cacheKey(identity))
}
